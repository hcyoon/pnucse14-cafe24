...
$(document).ready(function(){
    ...
    var methods = {
        ...
        show: function(overNode, iCateNo) {
            ...
            var divthemeclass = ($(".submenu-skeleton")[0].className.match(/userTheme[\w]+/)||[""])[0];
            var lithemeclass = ($(".submenu-skeleton li")[0].className.match(/userTheme[\w]+/)||[""])[0];
            var aHtml = [];
            aHtml.push('<ul>');
            $(methods.aSubCategory[iCateNo]).each(function() {

                aHtml.push('<li class="'+lithemeclass+'"><a href="'+this.link_product_list+'">'+this.name+'</a></li>');
            });
            aHtml.push('</ul>');


            var offset = $(overNode).offset();
            $('<div class="'+divthemeclass +' sub-category"></div>')
                .appendTo(overNode)
                .html(aHtml.join(''))
