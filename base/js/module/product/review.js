        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    // 비밀글 비밀번호 입력 폼
                    aHtml.push('<form name="SecretForm_4" id="SecretForm_4" class="secretForm">');
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=4&link_product_no='+iProductNo+'" class="btnNormal">게시글 수정하기</a>');
                    }
                    aHtml.push('</span></p>');
                    aHtml.push('</div>');
                    ...
                    if (data.comment != undefined && data.comment.length != undefined) {
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                aHtml.push('<span class="gIndent '+data.use_point+'"><img src="//img.echosting.cafe24.com/skin/base_ko_KR/board/ico_point'+data.comment[i]['comment_point_count']+'.gif" alt="'+data.comment[i]['comment_point_count']+'점" /></span>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<p class="captcha '+data.comment_write['use_captcha']+'">'+data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+data.comment_write['captcha']+'<img src="//img.echosting.cafe24.com/skin/base_ko_KR/product/ico_information.gif" alt="" />  왼쪽의 문자를 공백없이 입력하세요.(대소문자구분)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<p class="captcha '+data.comment_reply['use_captcha']+'">'+data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+data.comment_reply['captcha']+'<img src="//img.echosting.cafe24.com/skin/base_ko_KR/product/ico_information.gif" alt="" /> 왼쪽의 문자를 공백없이 입력하세요.(대소문자구분)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_secret != undefined) {
                        aHtml.push('<form name="commentSecretForm_4'+data.key+'" id="commentSecretForm_4'+data.key+'" class="" style="display:none">');
                ...
                $(pNode).after('<tr id="product-review-read'+data.key+'" class="commentSecretForm"><td colspan="6">'+aHtml.join('')+'</td></tr>');
